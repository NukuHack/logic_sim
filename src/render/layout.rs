...
pub fn calculate_min_chip_size(
    inputs: &[PinBitCount],
    outputs: &[PinBitCount],
    name: &str,
...
mod tests {
    ...
    fn mixed_bit_widths_use_correct_grid_heights() {
        ...
        // by +4.5.
        let (height, ys) =
            calculate_default_pin_layout(&[PinBitCount::Bit1, PinBitCount::Bit4, PinBitCount::Bit8]);
        assert_eq!(ys[0], 3.5);
        assert_eq!(ys[1], 1.0);
        assert_eq!(ys[2], -2.5);
    ...
    fn min_chip_size_widens_for_a_name_longer_than_the_pin_bounds() {
        ...
        let with_name = calculate_min_chip_size(
            &inputs,
            &outputs,
            "Full Adder",
            NameLocation::Centre,
        ...
        assert!(
            with_name.x > pins_only.x,
            "body should widen to fit the name label, not stay pin-sized"
        );
    ...
    fn min_chip_size_ignores_a_short_name_that_fits_within_pin_bounds() {
        ...
        let pins_only = calculate_min_chip_size_for_pins(&inputs, &outputs);
        let with_name =
            calculate_min_chip_size(&inputs, &outputs, "A", NameLocation::Centre, 0.25);
    ...
    fn min_chip_size_ignores_name_when_hidden() {
        ...
        let outputs = [];
        let pins_only = calculate_min_chip_size_for_pins(&inputs, &outputs);
        let with_hidden_name = calculate_min_chip_size(
            &inputs,
            &outputs,
            "A Very Long Chip Name Indeed",
            NameLocation::Hidden,
            0.25,
        );
    ...
    fn grid_line_thickness_never_renders_thinner_than_the_pixel_minimum() {
        for &zoom in &[0.05, 0.5, 1.0, 5.0, 50.0, 500.0, 4096.0] {
            ...
            assert!(
                screen_px >= GRID_MIN_PIXEL_THICKNESS - 1e-4,
                "zoom {zoom}: {screen_px}px thick, below the {GRID_MIN_PIXEL_THICKNESS}px minimum"
            );
        }
    }
