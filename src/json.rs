...
use crate::description::{
    ChipDescription, ChipLibrary, ChipType, Color, NameLocation, PinAddress, PinBitCount, PinDescription, SubChipDescription, ValueDisplayMode, WireConnectionType, WireDescription
};
...
fn to_chip_description(raw: &JsonChipDescription) -> ChipDescription {
    ...
    desc.input_pins = raw
        ...
        .iter()
        .map(|p| {
            PinDescription::from_saved(
                p.name.clone(),
                p.id,
                p.position,
                p.bit_count,
                p.colour,
                p.value_display_mode,
            )
        })
        .collect();
    ...
    desc.wires = raw
        ...
        .map(|w| {
            ...
            let bends = if w.points.len() > 2 {
                w.points[1..w.points.len() - 1].to_vec()
            } else {
                Vec::new()
            };

            WireDescription {
                source_pin_address: PinAddress::new(
                    w.source_pin_address.pin_owner_id,
                    w.source_pin_address.pin_id,
                ),
                target_pin_address: PinAddress::new(
                    w.target_pin_address.pin_owner_id,
                    w.target_pin_address.pin_id,
                ),
    ...
    let raw = JsonChipDescription {
        ...
        sub_chips: desc
            ...
            .map(|s| JsonSubChipDescription {
                ...
                pin_colour_info: Some(
                    s.pin_colour_info
                        .iter()
                        .map(|(pin_id, colour)| JsonPinColourInfo {
                            pin_colour: *colour,
                            pin_id: *pin_id,
                        })
                        .collect(),
        ...
        wires: desc
            ...
            .map(|w| JsonWireDescription {
                source_pin_address: JsonPinAddress {
                    pin_id: w.source_pin_address.pin_id,
                    pin_owner_id: w.source_pin_address.pin_owner_id,
                },
                target_pin_address: JsonPinAddress {
                    pin_id: w.target_pin_address.pin_id,
                    pin_owner_id: w.target_pin_address.pin_owner_id,
                },
                connection_type: w.connection_type,
                connected_wire_index: w.connected_wire_index,
                connected_wire_segment_index: w.connected_wire_segment_index,
...
pub fn load_chip_library_from_dir(chips_dir: &Path) -> std::io::Result<(ChipLibrary, Vec<String>)> {
    ...
    let mut entries: Vec<_> = fs::read_dir(chips_dir)?
        .filter_map(|e| e.ok())
...
impl ProjectDescription {
    ...
    pub fn is_starred(&self, chip_name: &str, is_collection: bool) -> bool {
        self.starred_list
            .iter()
            .any(|item| item.is_collection == is_collection && item.name.eq_ignore_ascii_case(chip_name))
    }
}
