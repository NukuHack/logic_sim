...
//!
//! A pin (or bus of up to 8 pins) is represented by `PinState`: a packed u16 under
//! the hood (tri-state flags in the high byte, bit values in the low byte), but
...
/// Packed state of up to 8 pins/wires: bit values in the low byte, tri-state
/// ("disconnected") flags in the high byte. This is the thing `SimPin::state`
/// stores, and what flows along wires/buses during simulation -- one byte per
/// half keeps every `SimPin` down to a size the stepping hot loop keeps in cache.
...
impl PinState {
	...
	pub const LOW: PinState = PinState::from_raw(LOGIC_LOW as u16);
	/// A single connected HIGH bit at index 0.
	pub const HIGH: PinState = PinState::from_raw(LOGIC_HIGH as u16);
	...
	pub const DISCONNECTED: PinState = PinState::from_parts(0, u8::MAX);
	...
	/// Builds directly from a bit-states byte and a tristate-flags byte (each bit
	/// `i` of `bit_states`/`tristate_flags` describes wire `i`).
	#[inline(always)]
	pub const fn from_parts(bit_states: u8, tristate_flags: u8) -> Self {
		Self((bit_states as u16) | ((tristate_flags as u16) << 8))
	}
	...
	pub fn set_bit(&mut self, index: u32, state: LogicState) {
		...
		let flag = bit << 8;
		match state {
			LogicState::Low => self.0 &= !(bit | flag),
			LogicState::High => {
				self.0 |= bit;
				self.0 &= !flag;
			}
	...
	pub fn toggle_bit(&mut self, index: u32) {
		self.0 = (self.0 ^ (1u16 << index)) & 0x00FF;
	}
}
