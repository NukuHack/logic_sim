use logic_sim::{
    load_chip_library_from_dir, load_project, ChipDescription, ChipLibrary, ChipType,
    ExternalInput, PinAddress, PinBitCount, PinDescription, Simulator, SubChipDescription,
    ValueDisplayMode, WireDescription, WireConnectionType, Color, NameLocation,
    parse_chip_description, serialize_chip_description, Vec2,
    ProjectDescription, ChipCollection, StarredItem,
};
use std::path::Path;
...
fn parses_not_chip_structure_correctly() {
    let (_project, library, errors) = load_chip_library_from_dir(&fixture_dir().join("Chips"))
        .map(|(lib, errs)| ((), lib, errs))
        .unwrap();
...
fn parses_wire_bend_points_from_saved_points_stripping_placeholder_ends() {
    let (_project, library, errors) =
        load_chip_library_from_dir(&gol_fixture_dir().join("Chips"))
            .map(|(lib, errs)| ((), lib, errs))
            .unwrap();
    assert!(errors.is_empty(), "parse errors: {errors:?}");
    ...
    let bent_wire = cell
        .wires
        .iter()
        .find(|w| w.points.len() == 4)
        .expect("expected to find the bent wire with 4 interior points");
...
fn serializes_project_description() {
    ...
    project.starred_list = vec![
        StarredItem::new("CHIP1", false),
        StarredItem::new("Collection", true),
    ];
    project.chip_collections = vec![
        ChipCollection::new("Basic", vec!["CHIP1", "CHIP2"]),
    ];
...
fn roundtrip_project_description() {
    ...
    original.all_custom_chip_names = vec![
        "NOT".to_string(),
        "AND".to_string(),
        "OR".to_string(),
    ];
    original.starred_list = vec![
        StarredItem::new("NOT", false),
        StarredItem::new("Collection", true),
    ];
    original.chip_collections = vec![
        ChipCollection::new("Basic", vec!["NOT", "AND"]),
        ChipCollection::new("Advanced", vec!["ADDER"]),
    ];
...
fn roundtrip_chip_with_all_enum_variants() {
    ...
    for color in [Color::Red, Color::Orange, Color::Yellow, Color::Green, 
    ...
    for mode in [ValueDisplayMode::None, ValueDisplayMode::Decimal, 
                 ValueDisplayMode::SignedDecimal, ValueDisplayMode::Hex] {
        original.output_pins.push(PinDescription {
            name: format!("{:?}", mode),
            id: mode.to_int(),
            position: Default::default(),
    ...
    for conn_type in [WireConnectionType::ToPins, WireConnectionType::ToWireSource, 
...
fn simulates_the_loaded_not_chip_correctly() {
    let (_project, mut library, errors) = load_chip_library_from_dir(&fixture_dir().join("Chips"))
        .map(|(lib, errs)| ((), lib, errs))
        .unwrap();
    assert!(errors.is_empty());
