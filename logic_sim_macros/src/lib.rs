...
pub fn derive_const_from_primitive(input: TokenStream) -> TokenStream {
    ...
    let data_enum = match &input.data {
        ...
        _ => {
            return syn::Error::new(Span::call_site(), "ConstFromPrimitive only supports enums")
                .to_compile_error()
                .into();
        }
    };
    ...
    for variant in &data_enum.variants {
        if !matches!(variant.fields, Fields::Unit) {
            return syn::Error::new_spanned(
                variant,
                "ConstFromPrimitive only supports fieldless (unit) variants",
            )
            .to_compile_error()
            .into();
        }
        ...
        let (_, discriminant) = variant.discriminant.as_ref().unwrap_or_else(|| {
            panic!(
                "variant `{}` needs an explicit discriminant (e.g. `= 0`)",
                variant.ident
            )
        });
    ...
    let default_ident = match default_ident {
        ...
        None => {
            return syn::Error::new(
                Span::call_site(),
                "ConstFromPrimitive requires one variant marked `#[default]`",
            )
            .to_compile_error()
            .into();
        }
